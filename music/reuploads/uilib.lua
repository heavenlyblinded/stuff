...
local function hideWindowDrawings()
    ...
    pcall(function()
        for _, t in ipairs(ProjectState.tabs) do
            if t._img then t._img.Visible = false end
            for _, sec in ipairs(t.sections) do for _, it in ipairs(sec.items) do if it._img then it._img.Visible = false end end end
        end
...
local function updateInput()
    ...
    local pollAll = ProjectState.focus ~= nil or ProjectState.spotlightOpen or ProjectState.dialog ~= nil or ProjectState.kbCapture ~= nil
    if not pollAll then
    ...
    if pollAll then
        ...
        local keys = {}
        keys[menuKey] = true
        keys.ctrl = true; keys.lctrl = true; keys.rctrl = true; keys.lalt = true; keys.space = true; keys.esc = true
...
local function drawDropdown(click, rightClick)
    ...
    if maxOff > 0 then
        ...
        rect(sbX - 1, dd._sbY - 2, 5, thumbH + 4, sCol, 205, 2.5, 0.18 * dd._sbHf * a)
        rect(sbX, dd._sbY, 3, thumbH, lerpColor(WHITE, sCol, 0.75), 205, 1.5, (0.25 + 0.45 * dd._sbHf) * a)
...
local function drawColorpicker(click, held)
    ...
    for i = 0, segs - 1 do
        ...
        if x1 > x0 then rect(x0, hueY, x1 - x0, 12, hueCols[i], 213, 0, a) end
    end
...
local function drawSpotlight(click)
    ...
    if maxOff > 0 then
        ...
        local sCol = lerpColor(Theme.accentA, Theme.accentB, sfr)
        rect(sbX - 1, thumbY - 2, 5, thumbH + 4, sCol, 404, 2.5, 0.18 * sp._sbHf * a)
        rect(sbX, thumbY, 3, thumbH, lerpColor(WHITE, sCol, 0.75), 404, 1.5, (0.25 + 0.45 * sp._sbHf) * a)
...
local function drawSectionCard(section, colX, sy, colW, secH, clipTop, clipBottom, click, held, rightClick, popupBlocking)
    ...
    if (section._cA or 0) > 0.98 then
        for _, it in ipairs(section.items) do if it._img then pcall(function() it._img.Visible = false end) end end
        return click, rightClick
    end
...
local function drawSections(tab, click, held, rightClick, px, contY, pw, contH)
    ...
    if tab.maxScroll > 0 then
        ...
        rect(trackX + 0.5, contY, 3, contH, WHITE, 34, 2, (0.05 + 0.05 * tab._sbGlow) * va)
        local bw = 3.5 + tab._sbGlow * 1.5
        local bx = trackX + 2 - bw / 2
        rect(bx - 2, barY - 3, bw + 4, barH + 6, barCol, 35, (bw + 4) / 2, 0.2 * tab._sbGlow * va)
        rect(bx, barY, bw, barH, barCol, 36, bw / 2, (0.55 + 0.45 * tab._sbGlow) * va)
        rect(bx + bw / 2 - 0.5, barY + 4, 1, max(1, barH - 8), WHITE, 37, 0.5, 0.16 * tab._sbGlow * va)
...
local function drawWindow(click, held, rightClick)
    ...
    if ProjectState.activeTab ~= ProjectState._tabSeen then
        ...
        pcall(function()
            for _, t in ipairs(ProjectState.tabs) do for _, sec in ipairs(t.sections) do for _, it in ipairs(sec.items) do if it._img then it._img.Visible = false end end end end
        end)
    ...
    if sideMode then
        ...
        if ProjectState.activeTab then
            txt(ProjectState.activeTab.name, x + sw + 16, textTop(y, topH, 15), WHITE, 15, FontBold, 13, false, false, max(2, (w - sw) - 272), AL.text * v)
        end
    else
    ...
    for i, tab in ipairs(ProjectState.tabs) do
        ...
        if abs(af - (active and 1 or 0)) < 0.004 then af = active and 1 or 0 end
        tab._af = af
        local hf = approach(tab._hf or 0, hov and 1 or 0, 14)
        if abs(hf - (hov and 1 or 0)) < 0.004 then hf = hov and 1 or 0 end
        tab._hf = hf
        local slide = 2.5 * hf * (1 - af)
        ...
        rect(x + 12, ty, sw - 24, pillH, WHITE, 61, 4, (0.07 * hf * (1 - af)) * v)
        ...
        if tab.icon then
            ...
            if IconBytes[tab.icon] then
                ...
                if tab._img then
                    pcall(function()
                        tab._img.Position = v2(x + 22 + slide, iiy); tab._img.Size = v2(16, 16)
                        tab._img.ZIndex = 629999; tab._img.Transparency = ia; tab._img.Visible = ia > 0.01
                    end)
                end
            else
                drawIcon(tab.icon, x + 22 + slide, iiy, 16, WHITE, 62, ia)
            end
            labelX = x + 48
        end
        txt(tab.name, labelX + slide, textTop(ty, pillH, 14), active and accentMidT or WHITE, 14, FontBold, 62, false, false, max(2, (x + sw - 18) - labelX), la * expand)
...
local function step()
    ...
    if Input.lctrl.held and Input.lalt.held and Input.space.click then
